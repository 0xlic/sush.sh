...
- 上传时读取远端目标文件当前大小；只有远端目标小于本地源文件总大小时，才将本地文件 seek 到相同偏移并以 append 模式继续写入
- 若目标侧大小等于或大于源文件大小，则判定为不可安全续传，回退为从 0 重传该文件
...
## SSH scrollback 与文本选择（v1.1）

v1.1 在嵌入式终端封装中补齐普通屏幕下的历史查看和本地复制能力：

- `ssh/terminal.rs` 维护 `display_offset`，普通屏幕下可从当前屏幕向 scrollback 历史移动视口
- `PageUp` / `PageDown` 和鼠标滚轮只在普通屏幕下由 TUI 捕获；alternate screen 下继续转发给远程程序
- 鼠标左键拖拽由 App 记录选区起止坐标，`ssh_view.rs` 在渲染时对选中 cell 加高亮
- 选区坐标按当前视口解析，因此可以覆盖当前屏幕和 scrollback 历史
- 鼠标释放时提取选中文本，并通过 crossterm clipboard support 写入系统剪贴板；失败时仅显示状态消息，不中断 SSH channel

## SFTP 导航增强（v1.1）

v1.1 在现有自适应双面板 SFTP 状态上增加当前激活面板范围内的导航输入：

- `Backspace` / `Left` 根据当前激活面板路径计算父目录，并分别调用本地文件系统或远程 SFTP 刷新列表
- `/` 打开当前目录列表过滤输入，只过滤已加载列表，不跨目录搜索；`Enter` 保留过滤结果，`Esc` 恢复完整列表
- `g` 打开目录跳转输入，绝对路径直接解析，相对路径基于当前激活面板目录解析
- 本地面板用本地文件系统验证目标目录，远程面板用 SFTP 列表请求验证目标目录
- 导航失败只更新状态消息，保持原路径、原列表和原选中状态
