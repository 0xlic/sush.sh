...
- [x] 文件夹递归传输（基础串行版本：保留目录本身 + 聚合进度）
- [x] 远程文件编辑：下载 → 默认图形化应用打开 → 监听变更 → 自动上传
- [x] 双面板模式（本地 + 远程同屏）
- [x] 文件多选批量操作（删除、传输）
- [x] 传输队列 + 后台传输（单连接 FIFO 队列 + 全局紧凑 badge）
- [x] 断点续传（普通文件按目标端现有大小续传）
...
## v1.1.0 — 终端与 SFTP 体验增强

- [x] SSH 嵌入式终端支持 scrollback，可滚动查看历史输出
- [x] SSH 嵌入式终端支持鼠标拖拽选中文本并复制到剪贴板
- [x] SFTP 界面支持返回上级目录
- [x] SFTP 界面支持当前列表模糊搜索
- [x] SFTP 界面支持 goto 跳转目录
