...
- 普通终端屏幕支持 scrollback、`PageUp`/`PageDown` 查看历史，以及鼠标选中复制
...
- `Enter` 进入目录，`Backspace`/`←` 返回上级，`/` 过滤当前列表，`g` 跳转目录
...
| 按键 | 动作 |
|------|------|
| `PageUp` / `PageDown` | 在普通屏幕模式下滚动查看终端历史 |
| 鼠标拖拽 | 选中文本，松开后复制到剪贴板 |
| `Ctrl-\` | 切换到 SFTP 文件浏览器 |
...
- 普通屏幕输出会保留 scrollback 历史，可用 `PageUp`/`PageDown` 和鼠标滚轮移动可视偏移
- 鼠标拖拽选区在本地高亮显示，并在松开时复制到系统剪贴板
