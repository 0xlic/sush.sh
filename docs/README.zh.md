...
首次启动时，`sush` 会自动导入 `~/.ssh/config` 中的主机。如果没有配置文件，从空列表开始（主机编辑 UI 在 v0.2 中上线）。
...
`sush` 使用**接管模式**，而不是嵌入式终端模拟器。连接主机后，`sush` 持有 I/O 通道，将字节直接在你的终端和远程 PTY 之间转发。这意味着：

- 终端程序的行为与原生 SSH 完全一致
- 没有额外的终端渲染开销
- `Ctrl-\` 被作为前缀键拦截；其他所有按键直接透传给远程
...
| v0.2 | TUI 主机编辑 · 标签管理 · 凭证加密存储 · 连接历史 |
| v0.3 | 文件夹递归传输 · 远程文件编辑 · 双面板 SFTP |
| v0.5 | 端口转发管理 · ProxyJump 多级跳板 · SOCKS5 代理 |
